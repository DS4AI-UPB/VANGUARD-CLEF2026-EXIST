...
class DemographicEncoder(nn.Module):
    """
    Encode categorical annotator demographics into a fixed-size vector.

    Maps gender, age, study level, and ethnicity through embedding layers,
    then averages across valid (non-padding) annotators
    """

    def __init__(self):
        super().__init__()
        self.gender_emb = nn.Embedding(4, 4, padding_idx=0)
        self.age_emb = nn.Embedding(4, 4, padding_idx=0)
        self.study_emb = nn.Embedding(7, 8, padding_idx=0)
        self.ethnicity_emb = nn.Embedding(8, 8, padding_idx=0)

    def forward(
            self, genders: torch.Tensor, ages: torch.Tensor, studies: torch.Tensor, eths: torch.Tensor
    ) -> torch.Tensor:
        mask = (genders != 0).float().unsqueeze(-1)
        g = self.gender_emb(genders)
        a = self.age_emb(ages)
        s = self.study_emb(studies)
        e = self.ethnicity_emb(eths)

        fused = torch.cat([g, a, s, e], dim=-1)
        sum_fused = (fused * mask).sum(dim=1)
        count_valid = mask.sum(dim=1).clamp(min=1)

        return sum_fused / count_valid


class SensorAutoencoder(nn.Module):
    """
    Pretrained to reconstruct the 4D sensor vector, then the encoder portion is injected into the main model
    to provide a learned 32D sensor representation
    """
...
class LoRAMemeModel(nn.Module):
    """
    Fusion strategy:
        - Cross-attention grounds description in visual patches
        - FiLM layers modulate the fused vision-language representation with human features (demographs and sensors)
        - Final classifier
    """

    def __init__(self, text_model: str, image_model: str, lora_r: int = 16, lora_alpha: int = 32):
        super().__init__()
    ...
    def forward(self, batch):
        t_out = self.text_encoder(input_ids=batch["text_input_ids"], attention_mask=batch["text_attention_mask"])
        t_seq = self.text_proj(t_out.last_hidden_state)
        t_cls = t_seq[:, 0, :]

        d_out = self.text_encoder(input_ids=batch["desc_input_ids"], attention_mask=batch["desc_attention_mask"])
        ...
        demo_feat = self.demo_encoder(batch["genders"], batch["ages"], batch["studies"], batch["eths"])
        ...
        return {
            "log_probs": torch.log_softmax(self.classifier(final_state), dim=1),
            "aux_sexism": self.aux_sexism_head(fused_cls),
